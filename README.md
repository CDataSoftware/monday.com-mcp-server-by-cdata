...
:heavy_exclamation_mark: This project builds a read-only MCP server. For full read, write, update, delete, and action capabilities and a simplified setup, check out our free [CData MCP Server for Monday.com (beta)](https://www.cdata.com/download/download.aspx?sku=MOZM-V&type=beta). 
## Purpose
We created this read-only MCP Server to allow LLMs (like Claude Desktop) to query live data Monday.com supported by the [CData JDBC Driver for Monday.com](https://www.cdata.com/drivers/monday/jdbc).
...
1. Clone the repository:
      ```bash
      git clone https://github.com/cdatasoftware/monday.com-mcp-server-by-cdata.git
      cd monday.com-mcp-server-by-cdata
      ```
2. Build the server:
      ```bash
      mvn clean install
...
2. Download and install the CData JDBC Driver for {source}: [https://www.cdata.com/drivers/monday/download/jdbc](https://www.cdata.com/drivers/monday/download/jdbc)
3. License the CData JDBC Driver:
    * Navigate to the `lib` folder in the installation directory, typically:
        * (Windows) `C:\Program Files\CData\CData JDBC Driver for Monday.com\`
...
5. Create a `.prp` file for your JDBC connection (e.g. `monday.com.prp`) using the following properties and format:
    ...
    * **Tables** - leave blank to access all data, otherwise you can explicitly declare the tables you wish to create access for
      ```env
      Prefix=monday
      ServerName=CDataMonday
      ServerVersion=1.0
      DriverPath=PATH\TO\cdata.jdbc.monday.jar
      DriverClass=cdata.jdbc.monday.MondayDriver
      JdbcUrl=jdbc:monday:InitiateOAuth=GETANDREFRESH;
      Tables=
      ```
